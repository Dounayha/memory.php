...
$cards = [];
$topScores = [];
$numberOfPairs = 6; // Nombre de paires de cartes

// Fonction pour obtenir les cartes
function getCards($numberOfPairs) {
    $images = [
    ...
    $images = array_slice($images, 0, $numberOfPairs); // Utiliser seulement le nombre de paires demandé
    $cards = array_merge($images, $images); // Double les images pour créer des paires
...
<?php if ($player): ?>
    ...
    <div class="card-container">
        <?php foreach ($cards as $index => $image): ?>
            <div class="card">
                <input type="checkbox" id="card<?php echo $index; ?>" class="card-checkbox">
                <label for="card<?php echo $index; ?>">
